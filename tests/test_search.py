...
class TestSubclassHandling:
    ...
    def test_subclass_depth_two_walks_deeper_hierarchy(self, graph, bmt):
        """With subclass_depth=2, walks two levels of hierarchy.

        Cardiovascular Disease (MONDO:0004995) has:
          -> Diabetes Mellitus (MONDO:0005015) at depth 1
            -> Type 2 Diabetes (MONDO:0005148) at depth 2

        Metformin treats both Type 2 Diabetes and Diabetes Mellitus.
        """
        query = {
            "message": {
                "query_graph": {
                    "nodes": {
                        "n0": {"ids": ["CHEBI:6801"]},
                        "n1": {"ids": ["MONDO:0004995"]},  # Cardiovascular Disease (grandparent)
                    },
                    "edges": {
                        "e0": {
                            "subject": "n0",
        ...
        # Depth 1 only reaches Diabetes Mellitus (child of Cardiovascular Disease)
        response_d1 = lookup(graph, query, bmt=bmt, verbose=False, subclass=True, subclass_depth=1)
        results_d1 = response_d1["message"]["results"]

        # Depth 2 also reaches Type 2 Diabetes (grandchild)
        response_d2 = lookup(graph, query, bmt=bmt, verbose=False, subclass=True, subclass_depth=2)
        results_d2 = response_d2["message"]["results"]

        # Depth 2 should find more results than depth 1
