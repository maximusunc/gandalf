...
class TestLookupOneHop:
    ...
    def test_one_hop_pinned_both_ends_single_result(self, graph):
        """Query with pinned start and end should return exactly 1 path."""
        query = {
            "message": {
                "query_graph": {
                    "nodes": {
        ...
        assert "n0" in results[0]["node_bindings"]
        assert "n1" in results[0]["node_bindings"]
...
class TestLookupEdgeCases:
    ...
    def test_category_filter_excludes_non_matching(self, graph):
        ...
        query = {
            "message": {
                "query_graph": {
                    "nodes": {
                        "n0": {"ids": ["CHEBI:6801"]},
                        "n1": {"categories": ["biolink:Disease"]},  # Only diseases
                    },
                    "edges": {
                        "e0": {
                            "subject": "n0",
        ...
        # Neither is a Disease, so should return 0
        assert len(results) == 0
