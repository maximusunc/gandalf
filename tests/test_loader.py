...
from gandalf.loader import build_graph_from_jsonl, _extract_qualifiers
...
class TestExtractQualifiers:
    """Tests for _extract_qualifiers supporting both qualifier formats."""

    def test_top_level_fields(self):
        """Should extract qualifiers from top-level fields."""
        data = {
            "subject": "CHEBI:6801",
        ...
        assert "biolink:object_aspect_qualifier" in type_ids
        assert "biolink:object_direction_qualifier" in type_ids
        values = {q["qualifier_value"] for q in qualifiers}
        assert "activity" in values
        assert "increased" in values

    def test_nested_qualifiers_array(self):
        """Should extract qualifiers from nested 'qualifiers' array."""
        data = {
            "subject": "CHEBI:6801",
            "object": "NCBIGene:3643",
            "predicate": "biolink:affects",
        ...
        assert "biolink:object_direction_qualifier" in type_ids
        values = {q["qualifier_value"] for q in qualifiers}
        assert "activity_or_abundance" in values
        assert "decreased" in values

    def test_top_level_takes_priority_over_nested(self):
        """Top-level fields should take priority when both formats are present."""
        data = {
            "subject": "CHEBI:6801",
            "object": "NCBIGene:3643",
            "predicate": "biolink:affects",
        ...
        qualifiers = _extract_qualifiers(data)
        # Top-level fields should be used, not nested
        assert len(qualifiers) == 2
        values = {q["qualifier_value"] for q in qualifiers}
        assert "activity" in values
        assert "increased" in values
    ...
    def test_no_qualifiers(self):
        """Should return empty list when no qualifiers present."""
        data = {
            "subject": "CHEBI:6801",
    ...
    def test_empty_nested_qualifiers(self):
        """Should return empty list when nested qualifiers is empty."""
        data = {
            "subject": "CHEBI:6801",
            "object": "MONDO:0005148",
            "predicate": "biolink:treats",
    ...
    def test_nested_qualifiers_skips_malformed_entries(self):
        """Should skip malformed entries in nested qualifiers array."""
        data = {
            "subject": "CHEBI:6801",
            "object": "NCBIGene:3643",
            "predicate": "biolink:affects",
            "qualifiers": [
                ...
                "not_a_dict",
                {"qualifier_type_id": "biolink:object_direction_qualifier"},  # missing value
                {"qualifier_value": "decreased"},  # missing type_id
            ],
        }
        qualifiers = _extract_qualifiers(data)
        assert len(qualifiers) == 1
        assert qualifiers[0]["qualifier_value"] == "activity"

    def test_nested_output_format_is_flat(self):
        """Nested qualifiers should be normalized to flat list of dicts."""
        data = {
            "subject": "CHEBI:6801",
            "object": "NCBIGene:3643",
            "predicate": "biolink:affects",
        ...
        # Each qualifier should be a flat dict with type_id and value
        for q in qualifiers:
            assert "qualifier_type_id" in q
            assert "qualifier_value" in q
            assert len(q) == 2
